...
def calculate_rhcoef(
    ...
    rhcoef = jax.vmap(LeafRHDL)(x)  # pyright: ignore
    rhcoef = rhcoef.reshape([Nt, Nlevel])
...
def leaf_ps_gsswc_hybrid(
    ...
    rh_leaf = eair_Pa / es(Tlk)  # need to transpose matrix
    rhcoef = calculate_rhcoef(prm, Tlk, eair_Pa, theta_soil)
    rh_leaf = rh_leaf * rhcoef
    # jax.debug.print("Tlk: {a}", a=Tlk[0,0])
    # jax.debug.print("eair_Pa: {a}", a=eair_Pa[0,0])
    # jax.debug.print("rhcoef: {a}", a=rhcoef[0,0])
    # jax.debug.print("rh_leaf: {a}", a=rh_leaf[0,0])
    ...
    def update_quad(
        ...
        def quad():
            ...
            eqx.error_if(
                product, jnp.sum(product < 0) > 0, "product must be nonnegative."
            )
