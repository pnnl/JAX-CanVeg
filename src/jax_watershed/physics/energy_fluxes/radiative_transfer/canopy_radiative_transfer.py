...
def calculate_canopy_fluxes_per_unit_incident(
    ...
    """Calculate the canopy radiative fluxes per unit incident radiation based on
       Section 3.1 in CLM5.
    ...
    Returns:
        Float_1D: Different components of canopy radiative fluxes per unit incident
                  radiation.
    """
    ...
    φ2 = 0.877 * (1 - 2 * φ1)
    K = G(φ1, φ2, μ) / μ
    μ_bar = 1.0 / φ2 * (1 - φ1 / φ2 * jnp.log((φ1 + φ2) / φ1))  # Eq(3.4) in CLM5

    # Calculate ω, β, β0
    # Note that τ_leaf, τ_stem, α_leaf, α_stem, and χl are obtained from Table(3.1) in
    #           CLM5, and ω_snow, β_snow, and β0_snow are obtained from Table(3.2) in
    ...
    #       where it should be max(μ*φ2+G(φ1,φ2,μ),1e-6) to avoid the impact of
    #       small values.
    ...
    as_μ = (
        ...
        * G(φ1, φ2, μ)
        / min_value
        * (1.0 - μ * φ1 / min_value * jnp.log((μ * φ1 + min_value) / (μ * φ1)))
    )  # Eq(3.16) in CLM5
    ...
    ωβ0 = (
        ω_veg * β0_veg * (1 - f_cansno) + ω_snow * β0_snow * f_cansno
    )  # Eq(3.7) in CLM5
    β, β0 = ωβ / ω, ωβ0 / ω
    ...
    (
        ...
        s1,
        s2,
    ) = calculate_auxiliary_variables(ω, β, β0, μ_bar, α_g_db, K, L, S)
    ...
    I_down_db = (
        h4 / σ * jnp.exp(-K * (L + S)) + h5 * s1 + h6 / s1
    )  # direct beam Eq(3.19)
    ...
    I_can_db = (
        1 - I_up_db - (1 - α_g_dif) * I_down_db - (1 - α_g_db) * I_down_trans_can
    )  # direct beam Eq(3.21)
...
def calculate_sunlit_shaded_plant_area_index(
    L: Float_0D, S: Float_0D, solar_elev_angle: Float_0D, pft_ind: int
...
def calculate_auxiliary_variables(
    ...
    h5 = (
        -1.0
        / d2
        * (h4 * (u2 + μ_bar * h) / (s1 * σ) + (u3 - h4 / σ * (u2 - μ_bar * K)) * s2)
    )
    h6 = (
        1.0
        / d2
        * (h4 / σ * (u2 - μ_bar * h) * s1 + (u3 - h4 / σ * (u2 - μ_bar * K)) * s2)
    )
    # print(h5, h6)
