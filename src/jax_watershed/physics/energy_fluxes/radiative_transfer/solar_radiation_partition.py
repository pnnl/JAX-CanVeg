...
def partition_solar_radiation(
    solar_rad: float, solar_elev_angle: float, pres: float
) -> Float_1D:
    """Partitioning solar radiation into direct and diffuse, visible and near-infrared components. 

    Args:
        solar_rad (float): the incoming solar radiation [W m-2]
    ...
    rdvis = 624. * jnp.exp(-0.185 * ru) * sine_solar_elev

    # Calculate potential diffuse PAR
    rsvis = 0.4 * (624.*sine_solar_elev - rdvis)

    # Calculate potential direct beam NIR
    wa = 1373.0 * 0.077 *jnp.power(2.*ru, .3) # water absorption in NIR for 10 mm precip water
    rdir = (748.0 * jnp.exp(-0.06*ru) - wa) * sine_solar_elev
    rdir = jnp.max(jnp.array([0, rdir]))

    # Calculate potential diffuse NIR
    rsdir = 0.6* (748.-rdvis/sine_solar_elev-wa)*sine_solar_elev
    ...
    cond2 = rit <= 0
    rvt   = jax.lax.cond(
        cond1, set_pointone, keep_as_is, rvt
    )
    rit   = jax.lax.cond(
        cond2, set_pointone, keep_as_is, rit
    )
    ...
    value = (0.9 - ratio) / 0.7
    # print(ratio, value)
    fdv   = rdvis / rvt * (1 - jnp.power(value, 2./3.))
    ...
    fdir  = rdir / rit * (1 - jnp.power(value, 2./3.))
