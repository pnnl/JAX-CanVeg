...
class Data(object):
    ...
    def _generate_minmax_scaler(self) -> None:
        self.min = jnp.nanmin(self.data, axis=tuple(i for i in range(1,len(self.shape))))
        self.max = jnp.nanmax(self.data, axis=tuple(i for i in range(1,len(self.shape))))
        # self.scaler = lambda x: (x-self.min) / (self.max-self.min)
        # self.inverse_scaler = lambda x: x*(self.max-self.min) + self.min

    def normalize(self, data: Array) -> Array:
        def normalize_each(x, xmin, xmax):
            return (x - xmin) / (xmax - xmin)
        return jax.vmap(normalize_each, in_axes=(0,0,0))(data, self.min, self.max)

    def inverse_normalize(self, data: Array) -> Array:
        def normalize_each(x, xmin, xmax):
    ...
    def interpolate_time_normalize_by_varn(self, varn: str, ts_interp: Float_1D) -> Float_1D:
        raise Exception('Not implmented.')
...
class DataBase(object):
    ...
    def __init__(self, forcings: Dict[str, AbstractGlobalInterpolation], dt: Float_0D) -> None:
