...
########################################################################
dir_data = Path(
    "/Users/jian449/Library/CloudStorage/OneDrive-PNNL/Data/AmeriFlux/AMF_US-Hn2_FLUXNET_FULLSET_2015-2018_3-5"
)
...
df_flux = pd.read_csv(f_flux_hh, index_col=0)
df_flux.index = pd.to_datetime(df_flux.index, format="%Y%m%d%H%M")
...
query = "".join([modis_baseurl, product, "/AMERIFLUX/", site, "/dates"])
...
df_obs["WS"] = df_obs["WS"] * 1.0  # [m s-1] -> [m s-1]
df_obs["TS"] = df_obs["TS"] + c2k  # [degC] -> [degK]
df_obs["SWC"] = df_obs["SWC"] * 1e-2  # [%] -> [-]
df_obs["LAI"] = df_obs["LAI"] * 1.0  # [-] -> [-]
