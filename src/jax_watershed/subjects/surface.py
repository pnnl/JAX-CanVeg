...
class Surface(BaseSubject):
    ...
    def __init__(
        ...
        # States
        self.states = {
            "l": l, "u_a" : u_a, "q_a" : q_a, "ρ_a" : ρ_a,
            "T_a" : T_a, "T_v" : T_v, "T_g" : T_g,
            "S_v" : S_v, "S_g" : S_g, "L_v" : L_v, "L_g" : L_g, "H_v" : H_v,
            "H_g" : H_g, "E_v" : E_v, "E_g" : E_g, "G"   : G,
        }

        self._parameter_initialize()
        self._state_initialize()
    
    def set_state_value(self, state_name: str, time_ind: int, space_ind: int, value: float):
        self.states[state_name] = self.states[state_name].at[(time_ind, space_ind)].set(value)

    def set_para_value(self, para_name: str, space_ind: int, value: float):
