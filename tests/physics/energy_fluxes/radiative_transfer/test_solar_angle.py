...
from jax_watershed.physics.energy_fluxes.radiative_transfer import (
    calculate_solar_elevation_Walraven,
)
...
class TestSolarAngle(unittest.TestCase):
    def test_solar_angle(self):
        ...
        print(
            "Solar elevation from calculate_solar_elevation: {}".format(solar_elevation)
        )
        print("")
        # self.assertIsInstance(solar_elevation, jnp.ndarray)
        self.assertIsInstance(solar_elevation, jax.Array)
    ...
    def test_solar_angle_walraven(self):
        ...
        print(
            "Solar elevation from calculate_solar_elevation_Walraven: {}".format(
                solar_elevation
            )
        )
        print("")
