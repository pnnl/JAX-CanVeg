...
from jax_watershed.physics.energy_fluxes.surface_energy import (
    leaf_energy_balance,
    calculate_Ts_from_TvTgTa,
    calculate_qs_from_qvqgqa,
)
from jax_watershed.physics.energy_fluxes.surface_energy import (
...
from jax_watershed.physics.energy_fluxes.radiative_transfer import (
    main_calculate_solar_fluxes,
)

# from jax_watershed.physics.energy_fluxes.turbulent_fluxes import *  # noqa: F403
...
class TestCanopyTemperature(unittest.TestCase):
    def test_estimate_canopy_temperature(self):
        print("Performing test_estimate_canopy_temperature()...")
        solar_rad, L_down, pres = 352.0, 200.0, 101.976
        ...
        q_v_sat_t2 = (0.622 * e_v_sat_t2) / (
            pres * 1e3 - 0.378 * e_v_sat_t2
        )  # [kg kg-1]
        # print(e_v_sat_t2, q_v_sat_t2, q_g_t2, q_a_t2)

        # Calculate the temperature and specific humidity of the canopy air/surface
        T_s_t2 = calculate_Ts_from_TvTgTa(
            Tv=T_v_t2, Tg=T_g_t2, Ta=T_a_t2, gam=gam, gvm=gvm, ggm=ggm
        )
        q_s_t2 = calculate_qs_from_qvqgqa(
            qv_sat=q_v_sat_t2, qg=q_g_t2, qa=q_a_t2, gaw=gaw, gvw=gvw, ggw=ggw
        )
        # print(T_s_t2, q_s_t2)

        # Solve the energy balance to estimate the vegetation temperature
        # print(leaf_energy_balance(T_v=T_v_t2, T_s=T_s_t2, q_v_sat=q_v_sat_t2*1e-3, q_s=q_s_t2*1e-3, gh=gvm, ge=gvw, S_v=S_v, L_v=L_v, ρ_atm=ρ_atm))  # noqa: E501
        args = dict(
            T_s=T_s_t2,
            q_v_sat=q_v_sat_t2,
            q_s=q_s_t2,
    ...
    def test_estimate_L_then_canopy_temperature(self):
        ...
        def func(x, args):
            return calculate_canopy_temp(x, **args)
    ...
    def test_estimate_canopy_temperature_and_L(self):
        print("Performing test_estimate_canopy_temperature_and_L()...")
        solar_rad, L_down, pres = 352.0, 200.0, 101.976
        L, S, pft_ind = 2.0, 1.0, 10
        latitude, longitude = 31.31, 120.77
        year, day, hour, zone = 2023, 68, 10.0, -8
        ...
        def func(x, args):
            return calculate_canopy_temp_L(x, **args)
        ...
        solver = NewtonNonlinearSolver(atol=atol, rtol=rtol)
        solution = solver(func, [L_guess, T_v_t2], args=args)
        L_est, T_v_t2_final = solution.root
        print("The guess and estimated L: {}, {}".format(L_guess, L_est))
        print(
            "The guess and estimated vegetation temperatures: {}, {}".format(
                T_v_t2, T_v_t2_final
...
def calculate_canopy_temp_L(
    ...
    # kwarg = dict(uz=u_a_t2, Tz=T_a_t2, qz=q_a_t2, Ts=ts, qs=qs, z=z, d=d, z0m=z0m, z0c=z0c)  # noqa: E501
    ...
    tstar = calculate_Tstar_most(
        T1=T_s_t2, T2=T_a_t2, z1=d + z0c, z2=z_a, d=d, ψc1=ψc_s, ψc2=ψc_a
    )  # [degK]
    qstar = calculate_qstar_most(
        q1=q_s_t2, q2=q_a_t2, z1=d + z0c, z2=z_a, d=d, ψc1=ψc_s, ψc2=ψc_a
    )  # [kg kg-1]
...
def calculate_canopy_temp(
    ...
    # uz=u_a_t2, Tz=T_a_t2, qz=q_a_t2, Ts=T_s_t2, qs=q_s_t2, z=z_a, d=d, z0m=z0m, z0c=z0c)  # noqa: E501
    # args = dict(uz=uz, tz=tz, qz=qz, ts=ts, qs=qs, z=z, d=d, z0m=z0m, z0c=z0c)
    def func(L, args):
